...
void test_f32_upscaling_rows(
    dsmvc::cuda_detail::CudaExecutor &cuda,
    const dsmvc::CpuExecutor &scalar) {
    constexpr std::int32_t destination_size = 1024;
    dsmvc::AxisPlan plan;
    plan.source_size = 1;
    ...
    plan.transpose_offsets.resize(
        static_cast<std::size_t>(destination_size) + 1U);
    plan.transpose_indices.assign(
        static_cast<std::size_t>(destination_size), 0);
    plan.transpose_weights.assign(
        static_cast<std::size_t>(destination_size), 1.0F);
    plan.inverse_diagonal.assign(
        static_cast<std::size_t>(destination_size), 1.0F);
    for (std::int32_t index = 0; index <= destination_size; ++index) {
        plan.transpose_offsets[static_cast<std::size_t>(index)] =
            static_cast<std::uint32_t>(index);
    }
    require(plan.valid(), "CUDA F32 upscaling fixture is invalid");
    test_rows(cuda, scalar, plan, "cuda-f32-upscaling-rows");
}
...
void test_device_conformance() {
    ...
    test_f32_upscaling_rows(cuda, scalar);
