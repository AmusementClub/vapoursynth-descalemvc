...
The report uses measured benchmark results as the authority for numeric claims:
...
4. The generated release report for the original `descale` comparison.
5. Git history for intent and rejected hypotheses only.
...
Old performance and paired error data were freshly measured, while current
Release performance was reused from the preceding current-only refresh.
Therefore the ratios are valid recorded release results, but this was not a
fully synchronous same-session rerun of both plugins.
...
4. Re-run the benchmark when changing the limiter or streamed-cache size;
   those parameters are hardware-sensitive.

## Appendix A. Evidence Index

| Evidence | Location |
|---|---|
| v4 throughput, memory profile, and CPU specialization | Summarized in this report |
| Original `descale` release comparison | [`release-benchmark.md`](release-benchmark.md) |
...
| Source/decision history | Git commits listed in Section 2 |
