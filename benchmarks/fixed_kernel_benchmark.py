...
def run_sample(options, kernel: dict, implementation: str,
    ...
    jet_plugin = (Path(options.jet_plugin).expanduser().resolve()
                  if getattr(options, "jet_plugin", "") else None)
    args = {
        ...
        "jet_plugin": str(jet_plugin) if jet_plugin else "",
    ...
    for kernel in kernels:
        for threads in sorted({item["threads"] for item in samples}):
            ...
            if "old" in implementations and "new" in implementations:
                ...
                item["speedup"] = None
            if "jet" in implementations and "new" in implementations:
                jet = item["jet"]["elapsed_seconds"]["median"]
                new = item["new"]["elapsed_seconds"]["median"]
                item["jet_speedup"] = jet / new
            else:
                item["jet_speedup"] = None
            result.append(item)
    return result
...
def write_scaling_svg(cases: list[dict], kernels: list[dict],
    ...
    labels = {
        ...
        "new": "new CPU",
        "newcpu": "new CPU",
        "newcuda": "new CUDA (explicit)",
        "newvulkan": "new Vulkan (explicit)",
...
def compact_speed(item: dict, implementations: tuple[str, ...]) -> str:
    ...
    return " / ".join(
        f"{implementation} {item[implementation]['fps']['median']:.3f}"
        for implementation in implementations)
...
def main() -> int:
    ...
    parser.add_argument("--jet-plugin", default="",
    ...
    if "jet" in options.implementations and (
        ...
        raise FileNotFoundError(f"jet plugin does not exist: {jet_plugin}")
    ...
    environment = {
        ...
        "jet_plugin": file_info(jet_plugin) if jet_plugin else None,
