...
def run_vspipe(options, case: str, implementation: str, run: int,
    ...
    args = {
        ...
        "jet_plugin": (str(Path(options.jet_plugin).expanduser().resolve())
                       if getattr(options, "jet_plugin", "") else ""),
...
def performance_summary(samples: list[dict], case: str,
    ...
    if "jet" in result and "new" in result:
        jet_median = result["jet"]["elapsed_seconds"]["median"]
        new_median = result["new"]["elapsed_seconds"]["median"]
        result["new_vs_jet_speedup"] = (jet_median / new_median
                                        if jet_median and new_median else None)
    else:
        result["new_vs_jet_speedup"] = None
    if "old" in result and "jet" in result:
        old_median = result["old"]["elapsed_seconds"]["median"]
        jet_median = result["jet"]["elapsed_seconds"]["median"]
        result["jet_vs_old_speedup"] = (old_median / jet_median
                                        if old_median and jet_median else None)
    else:
        result["jet_vs_old_speedup"] = None
    return result


def load_vs_plugins(core, options, need_old: bool = True,
                    need_new: bool = True,
                    reference: str = "old") -> None:
    ...
    if need_old and not hasattr(core, "descale"):
        reference_plugin = (options.jet_plugin if reference == "jet"
                            else options.old_plugin)
        if not reference_plugin:
            raise ValueError(
                f"reference '{reference}' requires its plugin path")
        core.std.LoadPlugin(path=str(Path(reference_plugin).resolve()))
...
def worker_errors(options) -> int:
    ...
    reference = getattr(options, "reference", "old")
    if reference not in REFERENCES:
        raise ValueError(f"unknown error reference: {reference}")
    ...
    load_vs_plugins(core, options, reference=reference)
    src8 = open_source(
...
def error_worker_command(options, case: str, output: Path,
    ...
    command = [
        ...
        "--jet-plugin", getattr(options, "jet_plugin", ""),
        "--reference", reference,
...
def run_error_worker(options, case: str, output: Path,
                     reference: str = "old") -> dict:
...
def write_error_csv(payloads: dict[str, dict], output: Path) -> None:
    fields = [
        "case", "profile", "reference", "frame", "index", "id", "scaler",
        "kernel",
        ...
        for case, payload in payloads.items():
            for row in payload["rows"]:
                writer.writerow({
                    ...
                    "reference": payload.get("reference", "old"),
                    "frame": payload["frame"],
                    "index": row["index"],
                    "id": row["id"],
                    "scaler": row["scaler"],
...
def write_report(result: dict, output: Path) -> None:
    ...
    speedup_columns = [
        ("new_speedup", "new vs old"),
        ("new_vs_jet_speedup", "new vs jet"),
        ("jet_vs_old_speedup", "jet vs old"),
    ]
    active_speedups = [
        (key, label) for key, label in speedup_columns
        if any(item.get(key) is not None
               for item in result["performance"]["summaries"])
    ]
    header_cells = (["Case", "Candidates"]
                    + [f"{implementation} median (s)"
                       for implementation in implementations]
                    + [label for _, label in active_speedups])
    divider_cells = (["---", "---:"]
                     + ["---:"] * len(implementations)
                     + ["---:"] * len(active_speedups))
    ...
    for item in result["performance"]["summaries"]:
        row = (f"| `{item['case']}` | {item['candidate_count']} | "
               + " ".join(
                   f"{item[implementation]['elapsed_seconds']['median']:.6f} |"
                   for implementation in implementations))
        for key, _ in active_speedups:
            value = item.get(key)
            row += f" {value:.3f}x |" if value is not None else " |"
        lines.append(row)
    if result["errors"]["summaries"]:
        lines.extend([
            ...
            "",
            "| Case | Reference | Best reference | Best new | Candidate changed | Max output abs | Max reconstruction abs |",
            "|---|---|---|---|---:|---:|",
        ])
        for item in result["errors"]["summaries"]:
            lines.append(
                f"| `{item['case']}` | `{item.get('reference', 'old')}` | "
                f"{item['best_old']['id']} "
        ...
        lines.extend([
            ...
            "",
            "| Case | Reference | Algorithm | Reference best height / MAE | New best height / MAE | Height changed |",
            "|---|---|---|---:|---:|---|",
        ])
        for item in result["errors"]["summaries"]:
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--jet-plugin", default=os.environ.get(
        "DSMVC_JET_PLUGIN", ""),
        help="JET vapoursynth-descale plugin; shares the descale namespace "
        "with the original, so it is always loaded into its own process.")
    ...
    result.add_argument(
        "--backend", choices=("auto", "cpu", "metal", "vulkan", "cuda"),
        default="cpu")
    ...
    result.add_argument("--reference", choices=REFERENCES, default="old",
                        help=argparse.SUPPRESS)
...
def main(options) -> int:
    ...
    jet_plugin = (Path(options.jet_plugin).expanduser().resolve()
    ...
    if "jet" in options.implementations and (
            jet_plugin is None or not jet_plugin.is_file()):
        raise FileNotFoundError(
            f"JET descale plugin does not exist: {jet_plugin}; "
            "pass --jet-plugin")
    ...
    references = [item for item in REFERENCES
                  if item in options.implementations]
    if (not options.skip_errors
            and ("new" not in options.implementations or not references)):
        raise ValueError(
            "error comparison requires new and at least one reference "
            "(old or jet); use --skip-errors")

    options.source = str(source)
    options.old_plugin = str(old_plugin) if old_plugin else ""
    options.jet_plugin = str(jet_plugin) if jet_plugin else ""
    ...
    environment = {
        ...
        "jet_plugin": str(jet_plugin) if jet_plugin else None,
        "jet_plugin_sha256": sha256_file(jet_plugin) if jet_plugin else None,
    ...
    if not options.skip_errors:
        for case in options.cases:
            for reference in references:
                error_file = output / (
                    f"errors-{case}.json" if reference == "old"
                    else f"errors-{case}.{reference}.json")
                payload = run_error_worker(
                    options, case, error_file, reference)
                error_payloads[f"{case}:{reference}"] = payload
                error_summaries.append({
                    "case": case, "reference": reference,
                    **error_summary(payload)})
                error_commands.append(payload["command"])
                print(
                    f"{case} error comparison vs {reference}: "
    ...
    result = {
        ...
        "errors": {
            ...
            "files": {key: (f"errors-{payload['case']}.json"
                            if payload.get("reference", "old") == "old"
                            else f"errors-{payload['case']}."
                                 f"{payload['reference']}.json")
                      for key, payload in error_payloads.items()},
        },
    }
