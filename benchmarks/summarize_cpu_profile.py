...
def render_markdown(data: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Bandwidth 5 and 7 remain on the generic vector path. At r32, Lanczos3 b5 spends {fmt(function_percent(b5_r32, 'solve_columns_'))}% in `solve_columns_vector<0>` and {fmt(function_percent(b5_r32, 'solve_horizontal_'))}% in `solve_horizontal_generic`; Spline64 b7 spends {fmt(function_percent(b7_r32, 'solve_columns_'))}% and {fmt(function_percent(b7_r32, 'solve_horizontal_'))}% respectively.",
